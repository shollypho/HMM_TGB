...
outdir = '/imaging/hp02/TGB/rest_closed/hmm_gamma/'; % path where data is located
...
for j = 1%length(all_data)
    ...
    if strcmp(all_data{j,1},'P9') && all_data{j,1}==2 % do not use head points
        coreg_settings.useheadshape = false;
    end
    coreg_settings.forward_meg = 'Single Shell';
    coreg_settings.use_rhino = true; %currently getting an error with rhino, wait to hear from david
    coreg_settings.fid.label.nasion='Nasion';
    coreg_settings.fid.label.lpa='LPA';
    coreg_settings.fid.label.rpa='RPA'; %%%  HP: ADD IN FIDUCIALS!
...
outdir = '/imaging/hp02/TGB/rest_closed/hmm_gamma';%fullfile( config.analysisdir,'spm_sss_processed' );
...
parfor j = 1:length(all_data)
    ...
    % Apply a 30-100Hz passband filter
    Dpar = osl_filter(Dpar,[30 100],'prefix','f');
    Dpar = osl_filter(Dpar,-[48 52],'prefix','f');
    Dpar = osl_filter(Dpar,-[98 102],'prefix','f');
...
%     D = spm_eeg_load(getfullpath(fullfile(outdir,sprintf('%s/fd%s.mat',all_data{j,1},all_data{j,19}))));
%     
%     copy(D, fullfile(outdir,sprintf('%s/icaman_fd%s.mat',all_data{j,1},all_data{j,19})));
   
    D = spm_eeg_load(getfullpath(fullfile(outdir,sprintf('%s/icaman_fd%s.mat',all_data{j,1},all_data{j,19}))));
    D = osl_africa(D,'do_ident','manual');
...
parfor j = 1:length(all_data)
    ...
    copy(D, fullfile(outdir,sprintf('%s/Nfffd%s.mat',all_data{j,1},all_data{j,19})));
    
    D = spm_eeg_load(getfullpath(fullfile(outdir,sprintf('%s/Nfffd%s.mat',all_data{j,1},all_data{j,19}))));
