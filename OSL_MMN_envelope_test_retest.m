...
outdir = '/imaging/hp02/TGB/rest_closed/hmm_test_retest/hmm_envelope_all/'; % path where outputs are located
...
for j = 1:length(all_data)
    % drug    
    %if strcmp(all_data{j,2}, 'Con') && all_data{j,7} == 1 && ~strcmp(all_data{j,1}, 'C17') && ~strcmp(all_data{j,1}, 'C6')% inclusion criteria in data_setup function.
    %plc
    %if strcmp(all_data{j,2}, 'Con') && all_data{j,7} == 0 && ~strcmp(all_data{j,1}, 'C17') && ~strcmp(all_data{j,1}, 'C4') && ~strcmp(all_data{j,1}, 'C15')% inclusion criteria in data_setup function.
    %all
    if strcmp(all_data{j,2}, 'Con') && ~strcmp(all_data{j,1}, 'C17') && j~=20 && j~=13 && j~=57 && j~=58 && ~strcmp(all_data{j,1}, 'C10') % inclusion criteria in data_setup function.
        ...
        %-------------------------------
        % continuous file
        D = spm_eeg_load(getfullpath(fullfile(megPath,sprintf('%s/hmm_1_45_Nicaman_fffd%s.mat',all_data{j,1},all_data{j,19}))));
...
options.K = K;  	  % The number of states to infer
...
nStates = K; % number of states to be used for hmm
