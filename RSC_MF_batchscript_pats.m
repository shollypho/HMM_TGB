...
subs2proc = pats_MF;  %Pats % [Cons_s1 Cons_s2];
% meg_badchans = ['0813 '] %[meg_badchans_cs1 meg_badchans_cs2];

dosubs =1:length(pats_MF) ;
...
for ss = 1:length(dosubs)
    ...
    megpath = strcat('/imaging/hp02/TGB/rest_closed/MF/Patients/', subs2proc{n},'/');
    ...
    allraw = spm_select('List', rawpath, '.*.fif'); %% all files
    ...
    allraw = allraw_rsc;
    Nses{ss} = size(allraw,1); %number of files
    Nses{5}=1;
...
%     cd('MF')
    ...
    for k = 1:Nses{ss} %raw file to process (sess1 etc)
        ...
        %load('/imaging/hp02/TGB/rest_closed/MF/patient_filenames_inclerrornames.mat');
...
for ss = 1:length(dosubs)
    load('/imaging/hp02/TGB/rest_closed/MF/patient_filenames_inclerrornames');
    load('/imaging/hp02/TGB/rest_closed/MF/pat_nses.mat');
    for k = 1:Nses{ss} %raw file to process (sess1 etc)
        n = dosubs(ss)
        
        %
        megpath = strcat('/imaging/hp02/TGB/rest_closed/MF/Patients/', subs2proc{n},'/');
        megpath_pf{ss}.(sprintf('k%s',num2str(k))) = strcat('/imaging/hp02/TGB/rest_closed/MF/Patients/', subs2proc{n},'/');
        ...
        for xyz =1:size(allbadch,2) %individual variables into strings
            ...
            badchstr_save{ss,k}.bad = badchstr;
...
%cd /imaging/hp02/TGB/rest_closed/MF
%save('badChans_pats.mat', 'badchstr_save');
%%
if maxsssflag
    %% SSS w movecomp
    parfor ss = 1:length(dosubs)
        for k = 1:Nses{ss}
            if sss1donealready{ss}.(sprintf('k%s',num2str(k))) < 1
                ...
                log_file = sprintf('%s_%d_ssslogfile.log',subs2proc{ss}, k);
                %copyfile(log_file, sprintf('movecomp_%s', log_file));
                %close all
            end
        end
    end
...
%     addfiff(sssfile2, strcat(rfxrootdir, '') ); %not the trans file
