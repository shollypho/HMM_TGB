...
outdir = '/imaging/hp02/TGB/rest_closed/hmm/hmm_envelope/'; % path where outputs are located
...
for j = 1:length(all_data)
    if all_data{j,22} == 1 % inclusion criteria in data_setup function.
        ...
        if j == 57 || j == 142 || j == 131% add envelope for rsc2 for these two participants b/c their rsc1 are too short
            
            if j ==131 % just need 5 more seconds, add again
            D2 = spm_eeg_load(getfullpath(fullfile(megPath,sprintf('%s/hmm_1_45_Nicaman_fffd%s.mat',all_data{j,1},all_data{j,19}))));
            else
            D2 = spm_eeg_load(getfullpath(fullfile(megPath,sprintf('%s/hmm_1_45_Nicaman_fffd%s.mat',all_data{j+1,1},all_data{j+1,19}))));
            end
...
options.K = nStates;  	  % The number of states to infer
